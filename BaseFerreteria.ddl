...
COMMENT ON COLUMN cargo.car_cargo IS
    'el cargo que puede ocupar los empleados ejemplo Cajero';

ALTER TABLE cargo ADD CONSTRAINT cargo_pk PRIMARY KEY ( car_codigo );
...
COMMENT ON COLUMN fer_categoria.cat_nombre IS
    'nombre de la categoria que se tiene del producto ejemplo decorado';

ALTER TABLE fer_categoria ADD CONSTRAINT fer_categoria_pk PRIMARY KEY ( cat_codigo );
...
COMMENT ON COLUMN fer_cliente.cli_nombre IS
    'nombre del cliente ejemplo ivan';

COMMENT ON COLUMN fer_cliente.cli_apellido IS
    'apellido del cliente ejemplo Sanchez';

COMMENT ON COLUMN fer_cliente.cli_direccion IS
    'direccion en donde vive el cliente ejemplo baños';

COMMENT ON COLUMN fer_cliente.cli_cedula IS
    'cedula o ruc del cliente ejemplo 0120115782';

COMMENT ON COLUMN fer_cliente.cli_email IS
    'correo electronico del cliente ejemplo ivan@hotmail.com';

COMMENT ON COLUMN fer_cliente.cli_num_tarjeta IS
...
COMMENT ON COLUMN fer_cliente.cli_genero IS
    'registra el genero que tiene el cliente ejemplo Masculino';

COMMENT ON COLUMN fer_cliente.cli_telefono IS
    'el numero de telefono o celular del cliente ejemplo 096878441';

ALTER TABLE fer_cliente ADD CONSTRAINT fer_cliente_pk PRIMARY KEY ( cli_codigo );
...
COMMENT ON COLUMN fer_empleado.emp_nombre IS
    'nombre del empleado ejemplo Rolando';

COMMENT ON COLUMN fer_empleado.emp_apellido IS
    'apellido del empleado ejemplo Calle';

COMMENT ON COLUMN fer_empleado.emp_direccion IS
    'direccion en donde vive el empleado ejemplo Ricaurte';

COMMENT ON COLUMN fer_empleado.emp_cedula IS
    'numero de cedula del empleado ejemplo 0258741369';

COMMENT ON COLUMN fer_empleado.emp_email IS
    'correo electronico del empleado el cual tambien servira como login ejemplo rolando@gmail.com';

COMMENT ON COLUMN fer_empleado.emp_contraseña IS
    'la contraseña del empleado al momento del login ejemplo Rc123';

COMMENT ON COLUMN fer_empleado.emp_genero IS
    'genero del empleado ejemplo Masculino';

COMMENT ON COLUMN fer_empleado.emp_telefono IS
    'el numero de telefono del trabajador ejemplo 0968718441';

ALTER TABLE fer_empleado ADD CONSTRAINT fer_empleado_pk PRIMARY KEY ( emp_codigo );
...
COMMENT ON COLUMN fer_factura_detalle.fac_det_cantidad IS
    'la cantidad de productos que se va a realizar la compra ejemplo 5';

COMMENT ON COLUMN fer_factura_detalle.fac_det_precio_uni IS
    'el precio del producto de forma individual ejemplo 12.20';

COMMENT ON COLUMN fer_factura_detalle.fac_det_precio_total IS
    'el precio total que se va a cancelar por los productos ejemplo 30.20';

ALTER TABLE fer_factura_detalle ADD CONSTRAINT fer_factura_detalle_pk PRIMARY KEY ( fac_det_codigo );
...
COMMENT ON COLUMN fer_producto.pro_nombre IS
    'el nombre del producto ejemplo Serrucho';

COMMENT ON COLUMN fer_producto.pro_descripcion IS
    'la descripcion del producto por ejemplo dientes de acero';

COMMENT ON COLUMN fer_producto.pro_costo_venta IS
    'el precio de venta al publico ejemplo 12.23';

COMMENT ON COLUMN fer_producto.pro_costo_compra IS
    'precio en el que se compro el producto ejemplo 12.50';

COMMENT ON COLUMN fer_producto.pro_cantidad IS
    'la cantidad que se tiene del stock';

COMMENT ON COLUMN fer_producto.pro_lugar_fabricacion IS
    'lugar de fabricacion ejemplo nacional o extranjero';

COMMENT ON COLUMN fer_producto.pro_iva IS
    'true si tiene iva el producto
false si no tiene iva el producto';

ALTER TABLE fer_producto ADD CONSTRAINT fer_producto_pk PRIMARY KEY ( pro_codigo );
...
COMMENT ON COLUMN fer_producto_proveedor.prod_prov_cantidad IS
    'la cantidad que provee el proveedor ejemplo 5';

ALTER TABLE fer_producto_proveedor ADD CONSTRAINT fer_producto_proveedor_pk PRIMARY KEY ( prod_pro_id );
...
COMMENT ON COLUMN fer_proveedor.prov_empresa IS
    'el nombre de la empresa que entrega los productos ejemplo Bellota';

COMMENT ON COLUMN fer_proveedor.prov_direccion IS
    'direccion del proveedor ejemplo Ordoñes laso';

COMMENT ON COLUMN fer_proveedor.prov_telefono IS
    'el telefono del proveedor ejemplo 2521255';

COMMENT ON COLUMN fer_proveedor.porv_nombre IS
    'el nombre de la persona que trabaja con el proveedor ejemplo Juan';

COMMENT ON COLUMN fer_proveedor.prov_apellido IS
    'apellido de la persona que tabaja con el proveedor ejemplo Perez';

COMMENT ON COLUMN fer_proveedor.prov_cedula IS
    'la cedula del trabajador del proveedor ejemplo 2012354894';

COMMENT ON COLUMN fer_proveedor.prov_email IS
    'el correo electronico del trabajador que provee';

COMMENT ON COLUMN fer_proveedor.prov_genero IS
    'el genero del proveedor ejemplo Masculino';

ALTER TABLE fer_proveedor ADD CONSTRAINT fer_proveedor_pk PRIMARY KEY ( prov_codigo );

CREATE TABLE fer_tipo_medida (
    med_codigo   NUMBER(5) NOT NULL,
    med_nombre   VARCHAR2(25) NOT NULL
);

COMMENT ON COLUMN fer_tipo_medida.med_nombre IS
    'nombre del tipo de medida que se tiene ejemplo Libras al ser en clavos o metros al ser sogas';

ALTER TABLE fer_tipo_medida ADD CONSTRAINT fer_tipo_medida_pk PRIMARY KEY ( med_codigo );
...
COMMENT ON COLUMN profesion.pro_profesion IS
    'profesion que puede tener los clientes ejemplo Arquitecto';

COMMENT ON COLUMN profesion.pro_descuento IS
    'el descuento que posee dicha profesion a momento de las compras ';

ALTER TABLE profesion ADD CONSTRAINT profesion_pk PRIMARY KEY ( pro_codigo );

ALTER TABLE fer_cliente
    ADD CONSTRAINT fer_cliente_profesion_fk FOREIGN KEY ( profesion_pro_codigo )
        REFERENCES profesion ( pro_codigo );

ALTER TABLE fer_empleado
    ADD CONSTRAINT fer_empleado_cargo_fk FOREIGN KEY ( cargo_car_codigo )
        REFERENCES cargo ( car_codigo );

--  ERROR: FK name length exceeds maximum allowed length(30) 
ALTER TABLE fer_factura_detalle
    ADD CONSTRAINT fer_fac_det_fer_fac_fk FOREIGN KEY ( fer_factura_fac_codigo )
        REFERENCES fer_factura ( fac_codigo );

--  ERROR: FK name length exceeds maximum allowed length(30) 
ALTER TABLE fer_factura_detalle
    ADD CONSTRAINT fer_fac_det_fer_pro_fk FOREIGN KEY ( fer_producto_pro_codigo )
        REFERENCES fer_producto ( pro_codigo );

ALTER TABLE fer_factura
    ADD CONSTRAINT fer_factura_fer_cliente_fk FOREIGN KEY ( fer_cliente_cli_codigo )
        REFERENCES fer_cliente ( cli_codigo );

ALTER TABLE fer_factura
    ADD CONSTRAINT fer_factura_fer_empleado_fk FOREIGN KEY ( fer_empleado_emp_codigo )
        REFERENCES fer_empleado ( emp_codigo );

ALTER TABLE fer_producto
    ADD CONSTRAINT fer_producto_fer_categoria_fk FOREIGN KEY ( fer_categoria_cat_codigo )
        REFERENCES fer_categoria ( cat_codigo );

--  ERROR: FK name length exceeds maximum allowed length(30) 
ALTER TABLE fer_producto
    ADD CONSTRAINT fer_pro_fer_tipMed_fk FOREIGN KEY ( fer_tipo_medida_med_codigo )
        REFERENCES fer_tipo_medida ( med_codigo );

--  ERROR: FK name length exceeds maximum allowed length(30) 
ALTER TABLE fer_producto_proveedor
    ADD CONSTRAINT fer_pro_prov_fer_pro_fk FOREIGN KEY ( fer_producto_pro_codigo )
        REFERENCES fer_producto ( pro_codigo );

--  ERROR: FK name length exceeds maximum allowed length(30) 
ALTER TABLE fer_producto_proveedor
    ADD CONSTRAINT fer_pro_prov_fer_prov_fk FOREIGN KEY ( fer_proveedor_prov_codigo )
        REFERENCES fer_proveedor ( prov_codigo );
